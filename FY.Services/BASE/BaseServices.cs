...
namespace FY.Services.BASE
{
    public class BaseServices<TEntity> : IBaseServices<TEntity> where TEntity : class, new()
    {
        ...
        public async Task<List<TEntity>> Query(
            ...
            PagerInfo pagerInfo)
        {
            return await BaseDal.Query(whereExpression, pagerInfo);
        }
        ...
        /// </summary>
        /// <param name="where">条件</param>
        /// <param name="pagerInfo">页码（下标0）</param>
        /// <param name="pageSize">页大小</param>
        /// <param name="orderByFileds">排序字段，如name asc,age desc</param>
        /// <returns>数据列表</returns>
        public async Task<List<TEntity>> Query(string where, PagerInfo pagerInfo)
        {
            return await BaseDal.Query(where, pagerInfo);
        }

        public async Task<PageModel<TEntity>> QueryPage(Expression<Func<TEntity, bool>> whereExpression, PagerInfo pagerInfo)
        {
            return await BaseDal.QueryPage(whereExpression, pagerInfo);
        }
    }
