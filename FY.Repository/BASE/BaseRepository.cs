...
using FY.Repository.UnitOfWorks;
using FY.IRepository.Base;
...
namespace Blog.Core.Repository.Base
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class, new()
    {
        ...
        {
            return await _db.Queryable<TEntity>()
                .OrderByIF(pagerInfo.Sort.IsNotEmptyOrNull(), $"{pagerInfo.Sort.ToSqlFilter()} {(pagerInfo.SortType.Contains("desc") ? "desc" : "asc")}"    )
                .WhereIF(whereExpression != null, whereExpression)
                .ToPageListAsync(pagerInfo.PageNum, pagerInfo.PageSize);
        }
        ...
        /// <param name="PagerInfo">分页查询对象</param>
        /// <returns></returns>
        public async Task<PageModel<TEntity>> QueryPage(Expression<Func<TEntity, bool>> whereExpression, PagerInfo pagerInfo)
        {
            ...
            var list = await _db.Queryable<TEntity>()
             .OrderByIF(pagerInfo.Sort.IsNotEmptyOrNull(), $"{pagerInfo.Sort.ToSqlFilter()} {(pagerInfo.SortType.Contains("desc") ? "desc" : "asc")}")
             .WhereIF(whereExpression != null, whereExpression)
             .ToPageListAsync(pagerInfo.PageNum, pagerInfo.PageSize, totalCount);

            return new PageModel<TEntity>(pagerInfo.PageNum, pagerInfo.PageSize, list, totalCount);
        }
            ...
            PagerInfo pagerInfo)
        {
            ...
            return new PageModel<TResult>(pagerInfo.PageNum, pagerInfo.PageSize, list, totalCount);
        }
