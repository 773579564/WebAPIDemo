...
namespace FY.Common.Helper
{
    ...
    public class ExcelHelper<T> where T : new()
    {
        ...
        public string ExportToExcel(string sDirectory, string sHeaderText, List<T> list, string[] columns)
        {
            string sFileName = string.Format("{0}_.xls", SecurityHelper.GetGuid(true));
            ...
            return Path.Combine(sDirectory, sFileName);
        }
        ...
        private MemoryStream CreateExportMemoryStream(List<T> list, string sHeaderText, string[] columns)
        {
            ...
            for (int rowIndex = 0; rowIndex < list.Count; rowIndex++)
            {
                ...
                for (int columnIndex = 0; columnIndex < properties.Length; columnIndex++)
                {
                    ...
                    if (sheet.GetColumnWidth(columnIndex) < length && !drValue.IsNullOrEmpty())
                    {
                    ...
                    switch (properties[columnIndex].PropertyType.ToString())
                    {
                        ...
                        case "System.Nullable`1[System.DateTime]":
                            newCell.SetCellValue(drValue.ObjToDateTime());
                    ...
                    if (mapPropertyInfoDict.ContainsKey(j))
                    {
                        if (row.GetCell(j) != null)
                        {
                            ...
                            switch (propertyInfo.PropertyType.ToString())
                            {
                                ...
                                case "System.Int16":
                                case "System.Nullable`1[System.Int16]":
                                    mapPropertyInfoDict[j].SetValue(entity, short.Parse(row.GetCell(j).ObjToString()));
                                    break;
                                case "System.Int32":
                                case "System.Nullable`1[System.Int32]":
